...
class Widget(ABC):
    ...
    def place_right_of(self, target: Widget, gap: int = 0) -> Self:
        return self.move_to(target.x + target.w + gap, target.y)

    def place_left_of(self, target: Widget, gap: int = 0) -> Self:
        return self.move_to(target.x - self.w - gap, target.y)

    def place_above(self, target: Widget, gap: int = 0) -> Self:
        return self.move_to(target.x, target.y - self.h - gap)

    def place_below(self, target: Widget, gap: int = 0) -> Self:
