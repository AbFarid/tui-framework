...
class Screen:
    ...
    def flush(self):
        if self._cursor_at is not None:
            x, y = self._cursor_at
            sys.stdout.write(self.term.move(y + self._oy, x + self._ox) + self.term.normal_cursor)  # type: ignore[arg-type]
    ...
    def put(self, x: int, y: int, text: str):
        sys.stdout.write(self.term.move(y + self._oy, x + self._ox) + text)  # type: ignore[arg-type]
